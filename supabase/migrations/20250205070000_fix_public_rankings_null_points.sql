-- Fix: Public rankings zeigt null Punkte für alle Teilnehmer
-- Problem: Die Funktion filtert nach qualification_start/end, aber wenn diese nicht gesetzt sind
-- oder Ergebnisse außerhalb liegen, werden keine Punkte gezählt.
...
begin
  ...
  -- Wenn qualification_start/end nicht gesetzt sind, verwende alle Ergebnisse
...
comment on function public.get_public_rankings(text, text) is
  'Returns top 50 ranking rows for public website by league and class. Callable by anon. If qualification_start/end are not set, all results are included.';
