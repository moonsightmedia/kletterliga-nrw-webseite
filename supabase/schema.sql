...
alter table public.profile_consents add column if not exists participation_terms_version text;
alter table public.profile_consents add column if not exists participation_terms_accepted_at timestamptz;
alter table public.profile_consents add column if not exists privacy_notice_version text;
alter table public.profile_consents add column if not exists privacy_notice_acknowledged_at timestamptz;
alter table public.profile_consents add column if not exists marketing_email_scope text;
alter table public.profile_consents add column if not exists marketing_email_status text not null default 'not_subscribed';
alter table public.profile_consents add column if not exists marketing_email_requested_at timestamptz;
alter table public.profile_consents add column if not exists marketing_email_confirmed_at timestamptz;
alter table public.profile_consents add column if not exists marketing_email_revoked_at timestamptz;
alter table public.profile_consents add column if not exists created_at timestamptz default now();
alter table public.profile_consents add column if not exists updated_at timestamptz default now();
alter table public.marketing_opt_in_tokens add column if not exists profile_id uuid references public.profiles(id) on delete cascade;
alter table public.marketing_opt_in_tokens add column if not exists token_hash text;
alter table public.marketing_opt_in_tokens add column if not exists token_type text;
alter table public.marketing_opt_in_tokens add column if not exists expires_at timestamptz;
alter table public.marketing_opt_in_tokens add column if not exists consumed_at timestamptz;
alter table public.marketing_opt_in_tokens add column if not exists created_at timestamptz default now();
...
begin
  ...
  if not exists (
    ...
    alter table public.profiles
      add constraint profiles_role_check check (role in ('participant','gym_admin','league_admin'));
  end if;
  if not exists (
...
create policy "Gym admins insert league" on public.gym_admins
  for insert with check (public.is_league_admin());
...
create policy "Finale registrations delete league" on public.finale_registrations
  for delete using (public.is_league_admin());

create index if not exists finale_registrations_profile_id_idx on public.finale_registrations (profile_id);
create index if not exists profile_consents_marketing_email_status_idx on public.profile_consents (marketing_email_status);
create index if not exists marketing_opt_in_tokens_profile_id_idx on public.marketing_opt_in_tokens (profile_id);
create unique index if not exists marketing_opt_in_tokens_token_hash_idx on public.marketing_opt_in_tokens (token_hash);
