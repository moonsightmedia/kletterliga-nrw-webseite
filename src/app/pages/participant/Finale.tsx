...
const Finale = () => {
  ...
  const canRegister = safelyLoaded && data?.eligible && data?.registration_open && !deadlinePassed && !busy;
          ...
          {status.isPending || !profile?.id ? <p role="status" className="py-4 text-sm">Dein Anmeldestatus wird geprüft …</p>
            : status.isError ? <div role="alert" className="semifinal-message space-y-3">
              ...
              : data?.registered ? <div className="space-y-3">
                <p className="flex items-center gap-2 text-lg font-bold">{data.eligible ? <CheckCircle2 className="h-6 w-6 shrink-0" aria-hidden="true" /> : <Clock3 className="h-6 w-6 shrink-0" aria-hidden="true" />}{data.eligible ? "Du bist angemeldet" : "Anmeldung gespeichert · Freigabe offen"}</p>
                ...
                : <div className="space-y-3">
                  ...
                  <StitchButton disabled={!canRegister} onClick={() => changeRegistration.mutate("register")} className="semifinal-primary">
