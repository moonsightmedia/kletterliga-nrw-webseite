...
export default function CompetitionDay() {
  ...
  const result = data?.results.find((item) => item.route_id === selectedRouteId) ?? null;
  const savedResult = (item: CompetitionRoute): SavedResult | undefined => data?.results.find((entry) => entry.route_id === item.id);
  const draftKey = profile?.id && season && route ? makeDraftKey(profile.id, season, route.id) : null;
  ...
  const persistDraft = (nextZone: number | null, nextFlash: boolean) => {
    if (!draftKey || nextZone === null || !data?.event || result || data.event.phase !== "open") return false;
    if (!Number.isInteger(nextZone) || nextZone < 0 || nextZone > 10 || (nextFlash && nextZone !== 10)) return false;
    try {
      window.localStorage.setItem(draftKey, JSON.stringify({ zone: nextZone, flash: nextFlash } satisfies Draft));
      setDraftSaved(true);
      setDraftError(null);
      return true;
    } catch {
  ...
  const submit = async () => {
    if (submitLockRef.current || !season || !profile?.id || !route || !qrToken || zone === null || !data?.event || result || data.event.phase !== "open") return;
    ...
    try {
      ...
      setSubmitError("Das Ergebnis wurde nicht bestätigt. Prüfe deine Verbindung und versuche es erneut.");
    } finally {
  ...
  const readOnly = data.event.phase !== "open";
  ...
  return <div className={pageClass}>
    <header className="grid gap-5 md:grid-cols-[1fr_auto] md:items-end">
      ...
      <StitchBadge tone={readOnly ? "ghost" : "navy"}>{readOnly ? data.event.phase === "draft" ? "NOCH NICHT GEÖFFNET" : "EINGABE GESCHLOSSEN" : "5 ROUTEN"}</StitchBadge>
    </header>
    {!readOnly && <p className="max-w-2xl text-sm leading-6 text-[#f2dcab]/75">Trage deinen letzten sicher gehaltenen Griff ein. Für eine Flash-Wertung muss die 10 im ersten Versuch erreicht sein. Öffne den Scanner hier in der App und zeige dem Routenposten deinen Bildschirm.</p>}
    <section aria-label="Deine Wettkampfrouten" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {data.routes.map((item) => {
        ...
        return <button type="button" key={item.id} disabled={submitting} onClick={() => { if (submitLockRef.current) return; setSelectedRouteId(item.id); setScannerOpen(false); setQrToken(null); }} aria-pressed={active} className={`rounded-xl p-4 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#f2dcab] focus-visible:ring-offset-2 focus-visible:ring-offset-[#003d55] disabled:opacity-60 ${active ? "bg-[#f2dcab] text-[#002637]" : "stitch-glass-card text-[#f2dcab] hover:bg-[rgba(242,220,171,0.12)]"}`}>
          <span className="flex items-center justify-between gap-3"><span className="stitch-headline text-2xl">Route {item.number}</span>{itemResult ? <Check aria-label="Ergebnis eingetragen" size={19} /> : <span aria-hidden="true" className="h-3 w-3 rounded-full" style={{ backgroundColor: item.color || "#a15523" }} />}</span>
          ...
          {item.color && <span className="mt-2 block text-xs opacity-75">Farbe: {item.color}</span>}
          {itemResult && <span className="mt-3 inline-flex items-center gap-1 text-xs font-bold"><Check size={14} /> Ergebnis eingetragen · {itemResult.points} Punkte</span>}
        </button>;
      })}
    </section>

    {route && <StitchCard tone="cream" className="space-y-5 p-5 sm:p-7" aria-labelledby="competition-route-title">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="stitch-kicker text-[#a15523]">DEINE WERTUNG</p><h2 id="competition-route-title" className="stitch-headline mt-2 text-2xl text-[#002637]">Route {route.number}: {route.name || "Halbfinalroute"}</h2></div>{result && <StitchBadge tone="terracotta"><Check size={14} /> ERGEBNIS EINGETRAGEN</StitchBadge>}</div>
      {result ? <div className="grid grid-cols-3 gap-2 sm:gap-3" aria-label="Eingetragenes Ergebnis"><div className="min-w-0 rounded-xl bg-white/70 p-2 sm:p-4"><span className="text-xs text-[#36515b]">Letzter Griff</span><strong className="stitch-headline mt-1 block text-2xl text-[#002637]">{result.zone}</strong></div><div className="min-w-0 rounded-xl bg-white/70 p-2 sm:p-4"><span className="text-xs text-[#36515b]">Flash</span><strong className="stitch-headline mt-1 block text-xl text-[#002637]">{result.flash ? "Ja" : "Nein"}</strong></div><div className="min-w-0 rounded-xl bg-white/70 p-2 sm:p-4"><span className="text-xs text-[#36515b]">Punkte</span><strong className="stitch-headline mt-1 block text-2xl text-[#002637]">{result.points}</strong></div></div> : readOnly ? <p className="text-[#36515b]">{data.event.phase === "draft" ? "Die Routen sind sichtbar. Die Ergebniseingabe ist noch nicht geöffnet." : "Für diese Route wurde kein Ergebnis eingetragen. Die Eingabe ist geschlossen."}</p> : <>
        <fieldset disabled={submitting} className="space-y-4 disabled:opacity-70"><legend className="mb-3 text-sm font-semibold text-[#002637]">Letzter sicher gehaltener Wertungsgriff</legend><div className="grid grid-cols-6 gap-2 sm:grid-cols-11">{Array.from({ length: 11 }, (_, value) => <button type="button" key={value} aria-pressed={zone === value} onClick={() => { const nextFlash = value === 10 ? flash : false; persistDraft(value, nextFlash); setZone(value); setFlash(nextFlash); setQrToken(null); }} className={`min-h-12 rounded-lg font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#003d55] ${zone === value ? "bg-[#003d55] text-[#f2dcab]" : "bg-white/75 text-[#002637] hover:bg-white"}`}>{value}</button>)}</div></fieldset>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap"><StitchButton variant="outline" disabled={zone === null || submitting} onClick={saveDraft}>Entwurf speichern</StitchButton><StitchButton className="whitespace-normal text-center tracking-[0.1em]" variant={qrToken ? "navy" : "outline"} disabled={zone === null || submitting} onClick={() => { if (submitLockRef.current) return; persistDraft(zone, flash); setScannerOpen(true); setQrError(null); }}><QrCode aria-hidden="true" size={17} />{qrToken ? "QR-Code erneut scannen" : "QR-Code am Routenposten scannen"}</StitchButton><StitchButton className="whitespace-normal text-center tracking-[0.1em]" disabled={!qrToken || zone === null || submitting || data.event.phase !== "open"} onClick={() => void submit()}>{submitting ? "Wird eingetragen …" : "Ergebnis absenden"}</StitchButton></div>
        {qrToken && <p className="flex items-start gap-2 text-xs leading-5 text-[#36515b]"><ShieldCheck aria-hidden="true" className="mt-0.5 shrink-0" size={16} />QR-Code erkannt. Prüfe deine Wertung und sende sie ausdrücklich ab. Der Scan allein trägt noch kein Ergebnis ein.</p>}
      </>}
