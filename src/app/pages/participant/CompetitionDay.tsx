  ...
  return <div className={pageClass}>
    ...
    <section aria-label="Deine Wettkampfrouten" className="space-y-3">
      {data.routes.map((item, index) => {
        ...
        const showRouteName = routeName && !/^(?:test\s*[·:–-]\s*)?route\s+\d+$/i.test(routeName);
        return <article key={item.id} className="overflow-hidden rounded-xl">
          <button type="button" disabled={submitting} onClick={() => { if (submitLockRef.current) return; setSelectedRouteId((current) => current === item.id ? null : item.id); setScannerOpen(false); setQrToken(null); }} aria-expanded={active} aria-controls={`competition-route-entry-${item.id}`} className={`block w-full p-4 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#a15523] disabled:opacity-60 ${active ? "bg-[#f2dcab] text-[#002637]" : "stitch-glass-card text-[#f2dcab] hover:bg-[rgba(242,220,171,0.12)]"}`}>
          <span className="flex items-center justify-between gap-3"><span className="stitch-headline text-2xl">Route {displayNumber}</span>{itemResult ? <Check aria-label="Ergebnis eingetragen" size={19} /> : <span aria-hidden="true" className="h-4 w-4 rounded-full ring-1 ring-black/20" style={{ backgroundColor: competitionRouteColor(item.color).value }} />}</span>
          {showRouteName && <span className="mt-2 block text-sm font-semibold">{routeName}</span>}
          {item.number !== displayNumber && <span className="mt-1 block text-xs opacity-75">Vor Ort: Route {item.number}</span>}
      ...
      {result ? <div className="grid grid-cols-2 gap-2 sm:gap-3" aria-label="Eingetragenes Ergebnis"><div className="min-w-0 rounded-xl bg-white/70 p-2 sm:p-4"><span className="text-xs text-[#36515b]">Letzter Griff</span><strong className="stitch-headline mt-1 block text-2xl text-[#002637]">{result.zone}</strong></div><div className="min-w-0 rounded-xl bg-white/70 p-2 sm:p-4"><span className="text-xs text-[#36515b]">Punkte</span><strong className="stitch-headline mt-1 block text-2xl text-[#002637]">{result.points}</strong></div></div> : readOnly ? <p className="text-[#36515b]">{data.event.phase === "draft" ? "Die Routen sind sichtbar. Die Ergebniseingabe ist noch nicht geöffnet." : "Für diese Route wurde kein Ergebnis eingetragen. Die Eingabe ist geschlossen."}</p> : <>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap"><StitchButton className="whitespace-normal text-center tracking-[0.1em]" variant={qrToken ? "navy" : "outline"} disabled={zone === null || submitting} onClick={() => { if (submitLockRef.current) return; persistDraft(zone); setScannerOpen(true); setQrError(null); }}><QrCode aria-hidden="true" size={17} />{qrToken ? "QR-Code erneut scannen" : "QR-Code am Routenposten scannen"}</StitchButton>{probeMode && <StitchButton variant="outline" className="whitespace-normal text-center tracking-[0.1em]" disabled={zone === null || submitting} onClick={() => { setQrToken("lokaler-probelauf"); setScannerOpen(false); setQrError(null); }}>Test-QR bestätigen</StitchButton>}<StitchButton className="whitespace-normal text-center tracking-[0.1em]" disabled={!qrToken || zone === null || submitting || (!probeMode && data.event.phase !== "open")} onClick={() => void submit()}>{submitting ? "Wird eingetragen …" : probeMode ? "Testwert speichern" : "Ergebnis absenden"}</StitchButton></div>
