...
const report = { synthetic: true, limits: 'Mocked APIs, no live data. Camera hardware, audio background behavior and real Supabase deployment are separate checks.', checks: [], errors: [] };
...
async function fixture(kind, width, initialPhase = 'open') {
  ...
  const state = { phase: initialPhase, opened: initialPhase !== 'draft', config: structuredClone(initialConfig), results: [], staff: [], writes: [], revoked: false };
  ...
  await context.addInitScript(({ session }) => { localStorage.setItem('sb-qa-post-qualification-auth-token', JSON.stringify(session)); sessionStorage.setItem('kl_app_sponsor_splash_seen', 'v2'); }, { session });
  await context.route('**/*', async (route) => {
    ...
    if (rpc === 'get_public_admin_settings') return reply([settings]);
...
try {
  for (const width of [320, 390, 768, 1440]) {
    ...
    await j.page.reload(); await j.page.getByRole('tab', { name: 'Routenuhren' }).waitFor();
    ...
    if (width === 390) {
      const decoded = await j.page.evaluate(async () => {
        ...
        try {
          ...
          return await scanner.scanFile(new File([blob], 'synthetic-qr.png', { type: 'image/png' }), false);
        } finally { scanner.clear(); div.remove(); }
      });
      check('QR image roundtrip decodes exact route credential', decoded === `${base}/app/wettkampf#route=${physical[0].id}&token=${physical[0].qr_token}`);
