...
try {
  for (const width of [320, 390, 768, 1440]) {
    // Focused admin QA is useful when participant/judge fixtures change independently.
    if (process.env.QA_ONLY_ADMIN !== '1') {
    ...
    await a.page.getByRole('button', { name: 'Eigene Farbe', exact: true }).click();
    await a.page.getByLabel('Farbcode (Hex)').fill('#xyz');
    check(`admin ${width}: invalid color blocked`, await a.page.getByRole('button', { name: 'Farbe übernehmen' }).isDisabled());
    await a.page.getByLabel('Farbcode (Hex)').fill('#123abc');
    await a.page.getByRole('slider', { name: 'Rot-Farbanteil' }).press('ArrowRight');
    check(`admin ${width}: keyboard color mixer`, await a.page.getByLabel('Farbcode (Hex)').inputValue() === '#133abc');
    await a.capture('custom-color');
    await a.page.getByRole('button', { name: 'Farbe übernehmen' }).click();
    await a.page.getByRole('button', { name: 'Änderungen speichern' }).click();
    await a.page.getByText('Routen und Klassen gespeichert.', { exact: true }).waitFor();
    check(`admin ${width}: custom color persisted`, a.state.config.routes[12].color === '#133abc');
    await a.page.reload();
    await a.page.getByRole('button', { name: 'Route 13 bearbeiten', exact: true }).click();
    await a.page.getByRole('button', { name: 'Eigene Farbe', exact: true }).click();
    check(`admin ${width}: custom color survives reload`, await a.page.getByLabel('Farbcode (Hex)').inputValue() === '#133abc');
    await a.page.getByLabel('Farbcode (Hex)').fill('#ff0000');
    await a.page.getByLabel('Farbcode (Hex)').press('Escape');
    check(`admin ${width}: dismiss discards unconfirmed color`, await a.page.getByRole('button', { name: 'Änderungen speichern' }).isDisabled());
    const entryMenu = a.page.getByRole('button', { name: /^Ergebniseingabe/ });
    check(`admin ${width}: operations initially collapsed`, await entryMenu.getAttribute('aria-expanded') === 'false');
    await entryMenu.scrollIntoViewIfNeeded();
    await a.capture('menus');
    await a.page.getByRole('button', { name: 'Route 1 bearbeiten', exact: true }).click();
    await a.page.getByRole('button', { name: /^Schiedsrichter/ }).click();
    ...
    check(`admin ${width}: only one operations menu expanded`, await a.page.getByRole('button', { name: /^Schiedsrichter/ }).getAttribute('aria-expanded') === 'false');
